...
def plot_mwd(ra,dec,label='',orig=0,title='Mollweide projection', projection='mollweide', target='', galactic_plane=True, dwarf_desy1=False, dwarf_desy2=False):
	x = np.remainder(ra+360-orig,360) # shift RA values
	ind = x>180
	...
	ax.scatter(x, y, marker='o', s=50, facecolor='red', edgecolor='', alpha=0.5)  # convert degrees to radians
	ax.set_xticklabels(tick_labels, alpha=0.3)     # we add the scale on the x axis
	...
	if type(target) == list:
		for t in target:
			...
			result = customSimbad.query_object(t)
			x=np.remainder(result['RA_d']+360-orig,360)
			ind = x>180
			x[ind] -=360    # scale conversion to [-180, 180]
			x=np.radians(-x)[0]    # reverse the scale: East to the left
			y=np.radians(result['DEC_d'])[0]

			ax.scatter(x, y, s=100, c='yellow', marker="*")
			ax.text(x, y-0.1, t, horizontalalignment='center', verticalalignment='top', fontsize=14)
	...
	if galactic_plane:
		...
		ax.plot(x[gv_sort],y[gv_sort],'--',color='blue')

	# DES Y1
	if dwarf_desy1:
		dwarf_data=Table(names=('name','ra','dec'), dtype=('S20','f8','f8'))
		
		dwarf_data.add_row(('Reticulum 2', 53.9256, -54.0492))
		dwarf_data.add_row(('Eridanus 2', 56.0878, -43.5332))
		dwarf_data.add_row(('Horologium 1', 43.8820, -54.1188))
		dwarf_data.add_row(('Pictoris 1 ', 70.9475, -50.2830))
		dwarf_data.add_row(('Phoenix 2', 354.9975, -54.4060))
		dwarf_data.add_row(('Indus 1', 317.2044, -51.1656))
		dwarf_data.add_row(('Grus 1', 344.1765, -50.1633))
		dwarf_data.add_row(('Eridanus 3', 35.6897, -52.2837))
		dwarf_data.add_row(('Tucana 2', 342.9796, -58.5689))

		x=np.remainder(dwarf_data['ra']+360-orig,360)
		ind = x>180
		x[ind] -=360    # scale conversion to [-180, 180]
		x=np.radians(-x)    # reverse the scale: East to the left
		y=np.radians(dwarf_data['dec'])

		ax.scatter(x, y, s=100, c='yellow', marker="*")
		for i in range(len(x)):
			ax.text(x[i], y[i]-0.02, dwarf_data['name'][i], horizontalalignment='center', verticalalignment='top', fontsize=8)

	# DES Y2
	if dwarf_desy2:
		dwarf_data=Table(names=('name','ra','dec'), dtype=('S20','f8','f8'))
		
		dwarf_data.add_row(('Grus 2', 331.02, -46.44))
		dwarf_data.add_row(('Tucana 3', 359.15, -59.60))
		dwarf_data.add_row(('Columba 1', 82.86, -28.03))
		dwarf_data.add_row(('Tucana 4', 0.73, -60.85))
		dwarf_data.add_row(('Reticulum 3', 56.36, -60.45))
		dwarf_data.add_row(('Tucana 4', 354.35, -63.27))
		dwarf_data.add_row(('Indus 2', 309.72, -46.16))
		dwarf_data.add_row(('Cetus 2', 19.47, -17.42))
